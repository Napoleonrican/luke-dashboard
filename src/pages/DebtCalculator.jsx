...
// ─── Debt source ──────────────────────────────────────────────────────────────
// Debts are now read live from the canonical `fin_debts` table (the Debts tab is
// the single source of truth). Each BNPL purchase — including the old Affirm
// sub-loans — is its own row there, so no separate Affirm breakdown is needed.
const TAG_FROM_CREDIT = { 'Credit Card': 'CC', Loan: 'Loan', BNPL: 'BNPL' };

// Per-debt line colors, assigned by position (cycled for long lists).
const PALETTE = [
...
export default function DebtCalculator() {
  ...
  // ── Scenario inputs (persisted to debt_settings — income/expense assumptions) ─
  ...
  // ── Live debts (single source of truth: fin_debts / the Debts tab) ──────────
  ...
  // Stable color per debt, assigned by position.
  const debtColor = useMemo(
    () => Object.fromEntries(debts.map((d, i) => [d.id, PALETTE[i % PALETTE.length]])),
    [debts],
  );

  const balancesUpdated = useMemo(() => {
    const dates = debts.map((d) => d.updated).filter(Boolean).sort();
    return dates.length ? fmtShortDate(dates[dates.length - 1]) : '—';
  }, [debts]);
    ...
    async function init() {
      ...
      if (data && !error) {
        ...
        if (!localTs || remoteTs > localTs) {
          // Only the scenario inputs live here now — debt figures come from fin_debts.
          ...
          try { localStorage.setItem(UPDATED_KEY, remoteTs); } catch { /* ignore */ }
        }
      }
  ...
  // ── Supabase push (debounced, gated by synced) — scenario inputs only ──────
        ...
        <section className="bg-zinc-900 border border-zinc-800 rounded-xl mb-4">
          ...
          {balancesOpen && (
            <div className="px-5 pb-5">
              <p className="text-xs text-zinc-500 mb-4 flex items-center gap-1.5 flex-wrap">
                Read-only — edit balances, APR &amp; minimums on the
                <Link to="/cashflow/debts" className="text-purple-400 hover:text-purple-300 inline-flex items-center gap-0.5">
                  Debts tab <ExternalLink size={11} />
                </Link>
              </p>
                  ...
                  return (
                    <div key={d.id}>
                      <div className="flex justify-between items-center mb-1.5">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-sm font-medium" style={{ color: debtColor[d.id] }}>{d.name}</span>
                        ...
                        <div className="flex items-center gap-4 text-xs tabular-nums">
                          <Redacted on={privacyMode}><span className="text-white font-medium font-mono">{fmtDec(d.balance)}</span></Redacted>
                          <Redacted on={privacyMode}><span className="text-zinc-400">{(d.apr * 100).toFixed(2)}%</span></Redacted>
                          <Redacted on={privacyMode}><span className="text-zinc-400 font-mono">{fmtDec(d.min)}/mo</span></Redacted>
                        </div>
                      </div>
              ...
              <div className="mt-4 pt-4 border-t border-zinc-800 flex justify-between text-sm">
                ...
                <span className="text-zinc-500 text-xs">
                  Starting baseline: <Redacted on={privacyMode}>{fmt(totalOriginal)}</Redacted>
                </span>
              </div>
            </div>
          )}
                ...
                <LineChart data={chartData} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
                  ...
                  {debts.map((d) => (
                    <Line key={d.id} type="monotone" dataKey={d.id} name={d.name}
                      stroke={debtColor[d.id]} dot={false} strokeWidth={1.5} />
                  ))}
