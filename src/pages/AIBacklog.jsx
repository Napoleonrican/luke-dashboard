...
import {
  Plus, ChevronDown, ChevronRight, ChevronUp, X, Check,
...
const SECTIONS = [
  { id: 'active_queue', label: 'Active Queue' },
  { id: 'research', label: 'Research & Analysis' },
  { id: 'writing', label: 'Writing & Drafts' },
  { id: 'decisions', label: 'Decisions & Thinking' },
  { id: 'gig_tracker', label: 'Gig Tracker — Business & Strategy' },
];
...
const SEED_TASKS = [
  ...
  },
  {
    ...
    priority: 'medium', owner: 'agent', status: 'pending',
...
const BLANK_FORM = { task_name: '', priority: 'medium', owner: 'agent', notes: '' };
...
export default function AIBacklog() {
  ...
  useEffect(() => {
    ...
    function close(e) {
      if (!e.target.closest('[data-status-dropdown]')) setOpenStatus(null);
    }
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="space-y-3">
          {SECTIONS.map(section => {
            const sectionActive = activeTasks.filter(t => t.section === section.id);
            const visible = sectionActive.filter(t =>
            ...
            return (
              <div key={section.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3">
                  ...
                  <button
                    onClick={() => { setAddingTo(addingTo === section.id ? null : section.id); setNewTask(BLANK_FORM); }}
                ...
                {!isCollapsed && (
                  <div className="border-t border-zinc-800">
                    {visible.length === 0 && addingTo !== section.id ? (
                      <p className="px-4 py-3 text-xs text-zinc-700">
                        {sectionActive.length === 0
                          ? 'Empty — add a task with +'
                        ...
                        {visible.map(task => {
                          ...
                          return (
                            <div key={task.id} className="px-4 py-2.5 hover:bg-zinc-800/25 transition-colors">
                              ...
                              {noted && task.notes && (
                                <div className="mt-2 ml-7 text-xs text-zinc-500 leading-relaxed bg-zinc-800/50 rounded px-2.5 py-2">
                                  {task.notes}
                                </div>
                              )}
                            </div>
                    ...
                    {addingTo === section.id && (
                      <div className="border-t border-zinc-700/80 bg-zinc-800/30 px-4 py-3 space-y-2.5">
                        <input
                          autoFocus
