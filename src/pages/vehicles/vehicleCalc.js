...
export function fuelStats(logs) {
  ...
  return sorted.map((log) => {
    const miles = prevOdo == null ? null : log.odometer - prevOdo;
    prevOdo = log.odometer;
    const gallons = log.gallons || 0;
    const pricePerGallon = gallons > 0 ? log.total_cost / gallons : null;
...
function aggregateBy(stats, keyFn) {
  ...
  for (const s of stats) {
    ...
    if (!key) continue;
    if (!map.has(key)) map.set(key, { key, miles: 0, gallons: 0, cost: 0 });
    const g = map.get(key);
  ...
  return [...map.values()]
    .map((g) => ({
      ...g,
      mpg: g.gallons > 0 ? g.miles / g.gallons : null,
      pricePerGallon: g.gallons > 0 ? g.cost / g.gallons : null,
